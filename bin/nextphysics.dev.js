...
//      checkEdges(object);
    ...
    function collisionProc(subject, subjectIdx){

        for(var j = subjectIdx + 1 ; j <objects.length; ++j){
            var object = objects[j];
           if(collisionCheck(subject, object))
           {
               if(!subject.isCollisioned) {
                   subject.isCollisioned = true;
                   subject.collisionOccur(object);
                   console.log(subject.id ,subject.force.z, object.id , object.force.z);
               }
           }
           else{
               subject.isCollisioned = false;
           }
        }
    }

    function collisionCheck(subject, object){
        var ret = false;
        if(subject.isRotation){
            var distance = Math.sqrt(Math.pow(subject.position.x - object.position.x, 2 ) + Math.pow(subject.position.y - object.position.y, 2) + Math.pow(subject.position.z - object.position.z, 2));
            ret = (distance < subject.radius + object.radius);
        }
        else {
            ret = subject.angle.z > object.angle.z;
        }
        return ret;
    }
  ...
  function updateVelocity(object, deltaT) {
    object.velocity.x += object.force.x * deltaT / object.mass;
    object.velocity.y += object.force.y * deltaT  / object.mass;
    object.velocity.z += object.force.z * deltaT / object.mass;
  }

  function updatePosition(object, deltaT) {
    if(object.isRotation == true){
        object.angle.x += object.velocity.x * deltaT;
        object.angle.y += object.velocity.y * deltaT ;
        object.angle.z += object.velocity.z * deltaT ;


        object.position.x = object.origin.x + Math.cos(object.angle.z) *object.penRadius;
        object.position.y = object.origin.y + Math.sin(object.angle.z) * object.penRadius;

        /*
        object.position.x = (Math.cos(object.angle.z) + Math.sin(object.angle.y))*object.penRadius;
        object.position.y = (Math.cos(object.angle.x) + Math.sin(object.angle.z))*object.penRadius;
        object.position.z = (Math.cos(object.angle.y) + Math.sin(object.angle.x))*object.penRadius;
        */
    }
    else{
...
NP.Pendulum = function(theta, radius, originX, originY, originZ, penRadius) {
    NP.Object.call(this);
    this.penRadius = penRadius;
    var x = this.penRadius*Math.cos(theta/180*Math.PI);
    var y = this.penRadius*Math.sin(theta/180*Math.PI);

    this.position = new THREE.Vector3(originX + x, originY + y, originZ);
    this.radius = radius !== undefined ? radius : 1;
    this.origin = new THREE.Vector3(originX, originY, originZ);
    this.isRotation = true;
    this.angle = new THREE.Vector3(0, 0, theta/180*Math.PI);
    this.mess = 30;
    this.id = originX;
    this.isCollisioned = false;
};

NP.Pendulum.prototype = Object.create(NP.Object.prototype);
NP.Pendulum.prototype.constructor = NP.Pendulum;

NP.Pendulum.prototype.renderScript = function(scene, doNotUse, updateFunctions) {
    var geometry = new THREE.SphereGeometry(this.radius);
    var sphere = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
        color: Math.random() * 0xffffff,
        wireframe: true
...
NP.Pendulum.prototype.collisionOccur = function(object){
   // this.force.x += this.k*(this.angle.x - object.angle.x )*this.penRadius*this.penRadius;
   // this.force.y += this.k*(this.angle.y - object.angle.y)*this.penRadius*this.penRadius;
   // var addForce = object.angle.z > this.angle.z ? (this.k*(  this.angle.z  - object.angle.z   ) + this.mu*( this.velocity.z - object.velocity.z ))*this.penRadius*this.penRadius : 0;
   // var addForce = -this.k*Math.abs( object.angle.z  - this.angle.z)*this.penRadius*this.penRadius;
   // this.force.z += addForce;
  //  object.force.z -= addForce;
    var temp = object.velocity.z;
    object.velocity.z = this.velocity.z;
